 ...
 */
UserService.getUserDetails = (userId, getDetailsCB) => {
  let user, role;
  async.waterfall([
 ...
 */
UserService.getUser = (email, password, getUserCB) => {
 ...
 */
UserService.signupUser = (user, signupCB) => {
  const id = uuidv4();
  user = {
    id,
    ...user,
 ...
 */
UserService.addUser = (user, addCB) => {
  async.waterfall([
 ...
 */
UserService.updateUserById = (user, updateCB) => {
 ...
 */
UserService.getUsersByRole = (roleId, getUsersCB) => {
  const query = {
    where: { roleId }
  };
 ...
 */
const loadUserDetails = (users, loadDetailsCB) => {
 ...
 * @param  {Function} getUsersCB
 */
UserService.getUsersByTenant = (tenantId, getUsersCB) => {
  async.waterfall([
 ...
 */
UserService.updateUserImage = (userId, image, updateUserCB) => {
  let user;
  async.waterfall([
    async.apply(userDao.findUserById, userId),
    (result, passIdCB) => {
