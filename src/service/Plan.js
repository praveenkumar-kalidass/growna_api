 ...
 */
const getPlanByQuotation = (quotation, getPlanCB) => {
...
PlanService.getPlanByQuotation = getPlanByQuotation;
 ...
 * @param  {Function} getCB
 */
const getPlanAndCompanies = (data, getCB) => {
  async.parallel({
    plan: getPlanByQuotation.bind(null, data),
    companies: getCompanies.bind(null, data.type)
  }, (parallelErr, result) => {
    if (parallelErr) {
 ...
 */
const calculatePlans = (quotation, plan, companies, calculateCB) => {
 ...
 */
PlanService.getPlans = (data, getPlansCB) => {
  async.waterfall([
    async.apply(getPlanAndCompanies, data),
    (result, passDataCB) => (
 ...
 */
PlanService.loadCartPlan = (quotation, company, loadPlanCB) => {
  async.waterfall([
    async.apply(getPlanByQuotation, quotation),
    (plan, passPlanCB) => (
